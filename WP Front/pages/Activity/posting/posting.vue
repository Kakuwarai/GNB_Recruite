<template>
            ...
            <div>
                ...
                <div class="flex flex-col p-2 py-6 w-80">
                    ...
                    <div class="bg-white items-center justify-between w-full flex rounded-full shadow-lg p-1 mb-5 sticky"
                        style="top: 5px">
                        <input v-model="txtSearch" @keypress.enter="getPosting(0)" 
                        ...
                        <div @click="getPosting(0)" class="bg-gray-600 p-2 hover:bg-blue-400 cursor-pointer mx-2 rounded-full">
            ...
            <div class="mt-1 relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 ">
                    ...
                    <tbody v-if="tblLoading == false">
                        <tr v-for="data in postingList" class="bg-white border-b  hover:bg-gray-50 ">
                            <td class="px-6 py-4">
                                {{ data.jobType }}
                            </td>
                            <td class="px-6 py-4">
                                {{ data.languageCodes.replaceAll('"', '').replaceAll('[', '').replaceAll(']', '')}}
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type="button" @click="editPosting(data)"
                                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-1.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                ...
                <el-empty v-if="postingList == '' && tblLoading == false" :image-size="200" />
            </div>
            <br>
  ...
  function handleCurrentChange(handleCurrentChange) {
    getPosting(handleCurrentChange - 1)
  }
  
  async function getPosting(page) {
    tblLoading.value = true
    console.log(userDetails.value);
    await globalAPI().post(`Posting/postingList?employeeId=${userDetails.value.id}&page=${page}&size=${pageSize.value}`)
        .then(async (response) => {
  
            if (response.status == 200) {

                postingList.value = response.data.postingList
                paginationPage.value = response.data.postingCount
  ...
  function addPosting() {
    router.replace('/activity/posting/addPosting')
  }
  
  function editPosting(editPosting) {
  
    localStorage.setItem('editTerms', JSON.stringify(editPosting))
    router.replace('/activity/posting/editPosting')
  }
