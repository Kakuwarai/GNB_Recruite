<template>
    ...
    <div class="flex flex-wrap">
        <div class="w-full rounded-3xl bg-white p-6 shadow :bg-gray-900">
            ...
            <el-select v-model="jobSelected" filterable placeholder="Add Job to tab" style="width: 240px">
                <el-option v-for="item in jobList" :key="item.code" :label="item.name" :value="item.code"
                    @click="addTab(jobSelected)" />

            </el-select>
            &nbsp;
            Filter by:
            <el-select v-model="filterBy" filterable placeholder="Select" style="width: 240px">
                <el-option @click="jobCandidates()" v-for="item in filterList" :key="item" :label="item"
                    :value="item" />
            </el-select>
            ...
            <el-tabs @click="event => { jobCandidates(), changeTabColor(targetName) }" v-model="editableTabsValue"
                type="card" class="demo-tabs" closable @tab-remove="removeTab">
                <el-tab-pane v-for="item in editableTabs" :key="item.name" :label="item.title" :name="item.name">

                    <!-- content -->
                    <div class="mt-1 relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table v-loading="loadingTable" :element-loading-spinner="svg"
                                ...
                                <tr v-for="data in jobCandidateList" class="bg-white border-b  hover:bg-gray-50 ">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <button type="button" @click="view(data)"
                                            ...
                                            <svg class="h-4 w-4 text-white-500" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
